...
> **AMENDED by §11 item 23 (era-relative recolor, shipped):** steps 1–2 above are the establishing outcome-coloured shot and stand. Steps 3–4 are re-staged as the **thesis beat** — the field recolours by era-relative intent (`uWallHeatMix` 0→1) so "the corner catches fire" is carried by the *colour* the outcome encoding couldn't show, and the chip becomes the single-column ball-1 reading ("struck at 74 in 2008, 96 now"). An always-visible diverging legend names the scale. See §11 item 23 for the exact shipped caption + legend copy.
...
23. **C1-2 era-relative recolor (owner-approved "colour by era, not outcome") — amends the C1-2 caption sequence in §3.** The wall's default outcome colouring (dot→blue … six→red) plus its (balls-faced → x) layout produces a strong left→right strike-rate gradient (batters accelerate once set — ball-1 SR ~74 climbing to ~140 by ball 20 in every era) that visually drowns out the chapter's actual thesis: the *early* balls got more aggressive over the YEARS (ball-1 SR 73.7 in 2008-10 → 95.5 in 2023-26; ball-5 110.8 → 145.5). Fix: keep the establishing outcome-coloured shot (steps 1–2) and the C1-5 six-fireworks two-tone UNCHANGED, and add a **THESIS BEAT** (steps 3–4) where each ball is recoloured by **era-relative intent** — how much hotter its (season × ball-index) cell strike rate is than the pooled IPL 2008-2010 batter at the SAME ball-index. Baselining every ball-index column to its own 2008-10 value REMOVES the horizontal gradient, so the early-ball corner visibly ignites bottom→top (recent seasons hot exactly where they most exceed 2008). Diverging scale: cool deep-blue (≈2008/below) → neutral grey-blue → amber → six-red at the top. New per-point buffer `wallheat.u8` (1 byte/point, neutral byte 73 = the 2008-10 batter); shader uniform `uWallHeatMix` (0 = outcome, 1 = era-relative); `SceneFieldState.wallHeatMix` ramps 0→1 during the wall scene's hold and settles back to 0 before the fireworks. Team-ignite still wins on top; reduced motion jump-cuts to the heated end-state (`wallHeatMix:1`).

    **Exact NEW caption copy shipped (for sign-off — supersedes §3 C1-2 steps 3–4):**
    - Step 1 (establishing, outcome colour — unchanged): *"Every ball, arranged by how long its batter had been in. **Left edge: a batter's first balls.** Rows climb from 2008 at the bottom to 2026 at the top. Brightness = runs."*
    - Step 2 (establishing, outcome colour — unchanged): *"**2008: the left edge is dark.** New batters blocked, nudged, had a look. The sighter, visible from space."*
    - Step 3 (**thesis beat** — `uWallHeatMix` ramps 0→1): *"Now colour each ball by how hard it's hit — versus a 2008 batter at the very same point. **The corner catches fire:** the opening balls, once played quietly, now blaze."*
    - Step 4 (**number chip**, heat held at 1; numbers read from `ch1.json ignition.sr_by_ball_index`, never hardcoded — 73.7→74, 95.5→96): *"**First ball of the innings: struck at 74 in 2008, 96 now.** ⓘ The revolution lives at the start of the innings."*

    **Legend copy shipped (always visible through the heat beat; names the scale on screen per the cognitive-design rule; labels read from `ch1.json ignition.wallheat.legend_labels`):** title *"Each ball vs a 2008 batter at the same point"* over a diverging swatch (deep-blue → grey-blue at the neutral pivot 73/255 → amber → six-red) with a neutral tick and the three stop labels — *"Below the 2008-10 batter" · "Like the 2008-10 batter" · "Hotter than the 2008-10 batter."* The `ignition-wall` footnote gains the recolor explanation (baseline = pooled 2008-10 SR per ball-count; cell = season × ball-index; why baselining each column cancels the acceleration gradient).
